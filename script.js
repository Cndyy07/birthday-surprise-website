document.addEventListener("DOMContentLoaded", () => {
      ...
      /* FUNGSI SFX TAMBAHAN (Agar tidak error saat dipanggil) */
      function playClickSFX() {
        console.log("SFX diputar");
        // Jika ada file audio klik, bisa dimasukkan ke sini
      }
      ...
      if (surpriseBtn) {
        surpriseBtn.addEventListener("click", () => {
          ...
          if (messageTarget) messageTarget.scrollIntoView({ behavior: "smooth" });
      ...
      function typeNextChar(text) {
        if (typingIdx < text.length) {
          if (typedTextElement) typedTextElement.textContent += text.charAt(typingIdx);
          typingIdx++;
      ...
      function extinguishCandle() {
        ...
        if (celebrateAgainBtn) celebrateAgainBtn.style.display = "inline-flex";
      }
      ...
      function resetCandle() {
        ...
        if (wishText) wishText.classList.remove("show");
        if (celebrateAgainBtn) celebrateAgainBtn.style.display = "none";
      ...
      if (saveWishBtn) {
        saveWishBtn.addEventListener("click", () => {
          if (!customWishInput || !generatedText) return;
